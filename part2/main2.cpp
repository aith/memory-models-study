...
bool t0_function(barrier_object *B) {
    ...
    auto Y = atomic_load_explicit(&y, memory_order_relaxed);
    atomic_store_explicit(&var1, Y,memory_order_relaxed);
    return true;
}
...
bool t1_function(barrier_object *B) {
    ...
    auto X = atomic_load_explicit(&x, memory_order_relaxed);
    atomic_store_explicit(&var2, X,memory_order_relaxed);
    return true;
}
